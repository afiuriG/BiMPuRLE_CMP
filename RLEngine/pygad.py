...
class GA:
            ...
            for gene_idx in mutation_indices:
                ...
                if self.gene_space_nested:
                    ...
                    # If the gene space is None, apply mutation by adding a random value between the range defined by the 2 parameters 'random_mutation_min_val' and 'random_mutation_max_val'.
                        ...
                        #this line was commented by Ariel
                        value_from_space = numpy.random.uniform(low=curr_gene_space['low'],
                                                                high=curr_gene_space['high'],
                                                                size=1)
                        #this code was added by Ariel
                        low=curr_gene_space['low']
                        high=curr_gene_space['high']
                        #value_from_space = self.arielMutation(low,high,offspring[offspring_idx, gene_idx])
                    else:
    ...
    def arielMutation(self, low, high,value):
        mean=0
        newval=0
        if high == 5.0:
            # mutate GLeak
            normalDistribuitedValue = numpy.random.normal(0, 0.2)
            newVal = value + normalDistribuitedValue
            if newVal < 0.05:
                newVal = 0.05
            elif newVal > 5.0:
                newVal = 5.0
            ...
            #mutate Vleak
            normalDistribuitedValue = numpy.random.normal(0, 10)
            newVal = value + normalDistribuitedValue
            if newVal < -90:
                newVal = -90
            elif newVal > 0:
            ...
            # mutate Cm
            normalDistribuitedValue = numpy.random.normal(0, 0.1)
            newVal = value + normalDistribuitedValue
            if newVal < 0.001:
                newVal = 0.001
            elif newVal > 1.0:
            ...
            # mutate Weight
            normalDistribuitedValue = numpy.random.normal(0, 0.5)
            newVal = value + normalDistribuitedValue
            if newVal < 0:
                newVal = 0
            elif newVal > 3.0:
